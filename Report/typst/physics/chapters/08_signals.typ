...
Process signals crossing between the simulator and the controller over OPC UA. Direction is
stated from the PLC's point of view. Every analog value is range-clamped before being written
(@sec-constants); optional per-signal first-order lag (0.3–2 s) and Gaussian noise are
...
Alarm and trip setpoints (used only to colour the P&ID and draw trend lines — the simulator
never evaluates them) and the extended Tier 2 discrete fault tags (scrubber level, vibration,
lubricator no-flow) are omitted from this summary; see `backend/config.yaml`'s `alarms` block
and `docs/APP_SPEC.md` §4.8 for the full lists.
